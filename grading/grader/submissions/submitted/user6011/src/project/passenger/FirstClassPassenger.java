...
public class FirstClassPassenger extends Passenger {
	...
	protected double calculateTicketPrice(Airport toAirport, double aircraftTypeMultiplier) {
		...
		double ticketPrice = 3.2*airportMultiplier*seatMultiplier*aircraftTypeMultiplier*connectionMultiplier*toAirport.getDistance(getDestinations().get(previousDisembarkIndex));
