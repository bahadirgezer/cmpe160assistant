...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Airline myAirline = new Airline(maxAircraftCount, operationFee, airportNumber, passengerNumber,
                                        prop, widebody, rapid, jet, outputFilePath);
        ...
        while(canFly){
            canFly = myAirline.findNextFlight(true);
        }
        if(myAirline.getRevenues()-myAirline.getExpenses() == 0.0){
            canFly = true;
            while(canFly){
                canFly = myAirline.findNextFlight(false);
            }
        }

        System.out.println("profit = " + (myAirline.getRevenues() - myAirline.getExpenses()));
    }
}
