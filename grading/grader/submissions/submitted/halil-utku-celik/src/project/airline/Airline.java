...
public class Airline {
    ...
    public int makeFlight(Airport fromAirport, Airport toAirport, boolean takeEconomy) throws IOException {
        ...
        for(Passenger passenger:fromAirport.getPassengers()){
            if(passenger instanceof EconomyPassenger && takeEconomy)
                continue;
            ...
            for(; destinationIndex < passenger.getDestinations().size(); destinationIndex++){
                if(passenger.getDestinations().get(destinationIndex) == toAirport){
                    ...
                    passengerWeight += passenger.getWeight();
        ...
        /*if(requiredEconomy + requiredBusiness * 3 + requiredFirstClass * 8 <= 30){
            ...
            if(!flag && fromAirport.canLand() && aircraftCount < maxAircraftCount){
                ...
                wr.write("0 " + fromAirport.getID() + " 3" + "\n");
                flag = true;
            ...
            if(!flag && fromAirport.canLand() && aircraftCount < maxAircraftCount){
                ...
                wr.write("0 " + fromAirport.getID() + " 0" + "\n");
                flag = true;
            ...
            if(!flag && fromAirport.canLand() && aircraftCount < maxAircraftCount){
                ...
                wr.write("0 " + fromAirport.getID() + " 1" + "\n");
                flag = true;
        ...
        wr.write("2 " + aircraft.getID() + " " + aircraft.getEconomySeats() + " " + requiredBusiness + " " + requiredFirstClass + "\n");

        for(Passenger passenger:passengersToFly){
            double expense = aircraft.loadPassenger(passenger);
            fromAirport.removePassenger(passenger);
        ...
        for(Airport airport:route){
            ...
            double expense = aircraft.addFuel(Double.max(0, optimisedFuel - fuelAmount)-0.002);
            makeExpense(expense);
            wr.write("3 " + aircraft.getID() + " " + Double.max(0, optimisedFuel - fuelAmount - 0.002) + "\n");
            double profit1=getRevenues()-getExpenses();
            fly(airport, aircraft.getID());
            double profit2=getRevenues()-getExpenses();
            wr.write("1 " + airport.getID() + " " + aircraft.getID() + "\n");
        }


        for (Passenger passenger:passengersToFly){
            double revenue = aircraft.unloadPassenger(passenger);
            makeRevenue(revenue);
            wr.write("5 " + passenger.getID() + " " + aircraft.getID() + " " + toAirport.getID() + "\n");
        }

        return 1;
    }

    public boolean findNextFlight(boolean takeEconomy) throws IOException {
        for(Airport airport:airports){
            int flightsMade = 0;
            ...
            for(Airport airport1:airports){
                if(airportCounter.containsKey(airport1.getID())){
                    if(airportCounter.get(airport1.getID())>0){
                        flightsMade += makeFlight(airport, airport1, takeEconomy);
                    }
                }
            }
            if(flightsMade > 0)
