...
public class Airline {
	...
	public boolean fly(Airport toAirport, int aircraftIndex) {
		...
		if (aircraftList.get(aircraftIndex).canFly(toAirport)) {
			...
			String log = "1 " + Integer.toString(toAirport.getID()) + " " + Integer.toString(aircraftIndex);
			logArray.add(log);
			return true;
		}
		else {
	 ...
	 */
	public double getAircraftCount() {
	...
	public boolean loadPassenger(Passenger passenger, Airport airport, int aircraftIndex) {
		if (!(airport.getPassengerList().containsValue(passenger))) {
			...
			String log = "4 " + Long.toString(passenger.getID()) + " " + Integer.toString(aircraftIndex)  + " " + Integer.toString(airport.getID());
			logArray.add(log);
			return true;
		}
		else {
	...
	public boolean unloadPassenger(Passenger passenger, int aircraftIndex) {
		if (passenger.canDisembark(aircraftList.get(aircraftIndex).getCurrentAirport())) {
			...
			String log = "5 " + Long.toString(passenger.getID()) + " " + Integer.toString(aircraftIndex)  + " " + Integer.toString(aircraftList.get(aircraftIndex).getCurrentAirport().getID());
			logArray.add(log);
			return true;
		}
		else {
	...
	public boolean createWideBodyPassengerAircraft(Airport airport) {
		if (getAircraftCount() + 1 <= maxAircraftCount && !airport.isFull()) {
			...
			this.aircraftList.add(aircraft);
			String log = "0 " + Integer.toString(airport.getID()) + " 1";
			logArray.add(log);
			return true;
		}
		return false;
	 ...
	 * @param firstClass first class seats to add
	 ...
	 */
	public boolean loadPassengersWithSameDestination(int aircraftIndex, Airport destination) {
		...
		for (Passenger passenger : passengerArray) {
			if (passenger.getDestinations().contains(destination)) {
				boolean b1 = this.loadPassenger(passenger, currentAirport, aircraftIndex);
				resBool = resBool || b1;
			}
		}
		return resBool;
	...
	public Airport closestHubAirporttoAircraft(int aircraftIndex) {
		...
		for (Airport airportDest : airportMap.values()) {
			if (currentAirport.getDistance(airportDest) < minDistance && airportDest.getAirportType() == 0 && !(this.aircraftList.get(aircraftIndex).getVisited().contains(airportDest))  && !airportDest.equals(currentAirport)) {
	...
	/**
	 * Gets the closest airport to an aircraft
	 * @param aircraftIndex Index of the aircraft
	 * @return The airport that is closest to the aircraft
	 */
	public Airport closestAirporttoAircraft(int aircraftIndex) {
		Airport currentAirport = this.aircraftList.get(aircraftIndex).getCurrentAirport();
		Airport mostClose = new HubAirport(1,1,1,1,1,1);
		double minDistance = Double.MAX_VALUE;
		for (Airport airportDest : airportMap.values()) {
			if (currentAirport.getDistance(airportDest) < minDistance && !(this.aircraftList.get(aircraftIndex).getVisited().contains(airportDest)) && !airportDest.equals(currentAirport)) {
				minDistance = currentAirport.getDistance(airportDest);
				mostClose = airportDest;
			}
		}
	...
	/**
	 * Flies a certain aircraft to nearest hub airport, if no hubs found nearby, then fly to a regular airport
	 * @param aircraftIndex Index of aircraft
	...
	public boolean flyToClosestHub(int aircraftIndex, int hubCounter) {
		...
		//if (!(this.getPassengersWithSameDestination(aircraftIndex, mostCloseHub))) return false;
		this.fillUp(aircraftIndex);
		if (!aircraft.canFly(mostCloseHub)) {
			mostCloseHub = closestAirporttoAircraft(aircraftIndex);
		}
